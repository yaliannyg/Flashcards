...
export const createCard = async ({ front, back, category }: CardDTO) => {
  const a = await CardModel.create({ front, back, category: category.id });
  console.log(a);
  return toCardDTO(a);
};

export async function getAllCards() {
  const cards = await CardModel.find().populate("category", "slug name");
...
export async function getCardsByCategories(slug: string) {
  ...
  if (!category) return [];
  const cards = await CardModel.find({ category: category._id }).populate(
    "category",
    "slug name",
  );
  return cards.map(toCardDTO);
...
export async function totalCards() {
  return await CardModel.countDocuments();
}
